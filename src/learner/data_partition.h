...
#ifndef UTBOOST_SRC_LEARNER_DATA_PARTITION_H_
#define UTBOOST_SRC_LEARNER_DATA_PARTITION_H_
...
#pragma omp parallel for schedule(static, 512) if (num_data_ >= 1024)
  ...
  void Split(int leaf, const Dataset* dataset, int feature,
             const uint32_t threshold, bool default_left,
             int right_leaf) {
    // get leaf boundary
    ...
    const auto left_cnt = runner_.Run<false>(
        ...
        [=](int, data_size_t cur_start, data_size_t cur_cnt, data_size_t* left,
            data_size_t* right) {
          return dataset->Split(feature, threshold, default_left,
...
#endif //UTBOOST_SRC_LEARNER_DATA_PARTITION_H_
